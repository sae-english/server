...
public interface SeriesRepository extends JpaRepository<SeriesEntity, Long> {
    ...
    Optional<SeriesEntity> findByContentKey(String contentKey);
    ...
    @Query("SELECT s FROM SeriesEntity s WHERE s.id IN :ids")
    List<SeriesEntity> findByIdIn(@Param("ids") List<Long> ids);
}
